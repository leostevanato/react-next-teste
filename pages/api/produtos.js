...
export default async function handler(req, res) {
	...
	if (req.method === "POST") {
		const productName = req.body.produto_nome;
		const addProducts = await query({
			query: "INSERT INTO produtos (produto_nome) VALUES (?)",
			values: [productName],
		});
		let product = [];
	...
	if (req.method === "PUT") {
		...
		const updateProducts = await query({
			query: "UPDATE produtos SET produto_nome = ? WHERE produto_id = ?",
	...
	if (req.method === "DELETE") {
		...
		const deleteProducts = await query({
			query: "DELETE FROM produtos WHERE produto_id = ?",
			values: [productId],
		});
